...
@DistributedSaga(
    ...
    successStatus      = "CONFIRMED",           // REQUIRED. Status set on the entity when saga succeeds.
    failureStatus      = "CANCELLED"            // REQUIRED. Status set on the entity when saga fails.
)
...
- `successStatus` and `failureStatus` are **required** — omitting either causes a compile error. Use the raw string value (e.g. `"ACTIVE"`) for `String` status fields, or prefix with the enum class name (e.g. `"OrderStatus.CONFIRMED"`) for typed enum fields.
...
- On success: sets the entity's status to the value of `successStatus` declared on `@DistributedSaga`.
- On failure: sets the entity's status to the value of `failureStatus` declared on `@DistributedSaga`.
- Both `successStatus` and `failureStatus` support plain String values (e.g. `"CONFIRMED"`) and qualified enum constants (e.g. `"OrderStatus.CONFIRMED"`) — the generator emits the correct Java syntax and adds the import automatically.
