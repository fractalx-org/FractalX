 ...
 *   logger-service/
 *     pom.xml
 *     src/main/java/com/fractalx/logger/
...
class LoggerServiceGeneratorSpec extends Specification {
    ...
    private Path javaRoot()       { serviceRoot().resolve("src/main/java/com/fractalx/logger") }
    ...
    def "LoggerApplication is a @SpringBootApplication main class in com.fractalx.logger"() {
        when:
        generator.generate(outputRoot)

        then:
        def c = application()
        c.contains("package com.fractalx.logger;")
