...
class AdminDataConsistencyGeneratorSpec extends Specification {
    ...
    def "generates two files in the data package"() {
        ...
        def pkg = srcMainJava.resolve("org/fractalx/admin/data")
    ...
    def "SagaMetaRegistry is in org.fractalx.admin.data package"() {
        when:
        generator.generate(srcMainJava, BASE, [order])

        then:
        sagaRegistry().contains("package org.fractalx.admin.data")
    }
    ...
    def "DataConsistencyController is in org.fractalx.admin.data package"() {
        when:
        generator.generate(srcMainJava, BASE, [order])

        then:
        dataController().contains("package org.fractalx.admin.data")
    }

    // helpers
    private String sagaRegistry() {
        Files.readString(srcMainJava.resolve("org/fractalx/admin/data/SagaMetaRegistry.java"))
    }

    private String dataController() {
        Files.readString(srcMainJava.resolve("org/fractalx/admin/data/DataConsistencyController.java"))
    }
}
