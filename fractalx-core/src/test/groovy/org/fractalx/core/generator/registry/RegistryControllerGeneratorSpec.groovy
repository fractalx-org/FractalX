...
class RegistryControllerGeneratorSpec extends Specification {
    ...
    private Path controllerFile() {
        srcMainJava.resolve("org/fractalx/registry/controller/RegistryController.java")
    }
    ...
    def "controller is in the org.fractalx.registry.controller package"() {
        when:
        generator.generate(srcMainJava)

        then:
        content().startsWith("package org.fractalx.registry.controller;")
    }
